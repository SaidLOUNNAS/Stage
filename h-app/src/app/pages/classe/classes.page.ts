...
export class ClassesPage extends Base implements OnInit {
  ...
  constructor(injector: Injector, private authService: AuthService, private classeService: ClasseService) {
    super(injector);
  }

  async ngOnInit() {
    this.user = this.authService.getCurrentUser();
    this.userRole = await this.classeService.getRole(this.user);
    this.getUsers();
  }
  ...
  async onShowDetails(id: string) {
    ...
    try {
      const user = await this.classeService.getClasse(id);
  ...
  private async getUsers() {
    try {
      ...
      const users = await this.classeService.getClasses(this.params);
