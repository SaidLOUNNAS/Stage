...
export class UsersPage extends Base implements OnInit {
  ...
  constructor(injector: Injector, private authService: AuthService, private userService: EtudiantService) {
    super(injector);
  }

  async ngOnInit() {
    this.user = this.authService.getCurrentUser();
    this.userRole = await this.userService.getRole(this.user);
    this.getUsers();
  }
  ...
  async onDelete(id: string) {
    ...
    alert.onDidDismiss().then(async (result) => {
      if (result.role === 'ok') {
        try {
          await this.userService.deleteEtudiant(id);
          this.getUsers();
  ...
  private async getUsers() {
    try {
      ...
      const users = await this.userService.getEtudiants(this.params);
